...
def _maybe_shift_logits(logits, shift):
    """Shift logits right by 1 position for next-token prediction models (e.g. DREAM)."""
    if shift:
...
def generate(
    ...
    for num_block in range(num_blocks):
        ...
        while True:
            ...
            logits = _maybe_shift_logits(model(x).logits, shift_logits)
...
def generate_with_prefix_cache(
    ...
    for num_block in range(num_blocks):
        ...
        full_logits = _maybe_shift_logits(output.logits, shift_logits)
        ...
        if factor is None:
            ...
            x0, transfer_index = get_transfer_index_dynamic(
                full_logits, temperature, remasking, mask_index, x, None, factor
            )
        x[transfer_index] = x0[transfer_index]
        ...
        for i in range(len(past_key_values)):
            ...
            for j in range(len(past_key_values[i])):
                kv = past_key_values[i][j]
                if kv.ndim == 4:
                    # 4D cache (batch, heads, seq_len, head_dim) — e.g. LLaDA
                    new_past_key_values[i] += (kv[:, :, :current_block_start],)
                else:
                    # 3D cache (batch, seq_len, hidden_size) — e.g. DREAM
                    new_past_key_values[i] += (kv[:, :current_block_start, :],)
...
def generate_with_dual_cache(
    ...
    for nb in range(num_blocks):
        ...
        out_full = model(x, use_cache=True, **extra_model_kwargs)
        ...
        full_logits = _maybe_shift_logits(out_full.logits, shift_logits)

        if factor is None:
            quota0 = None if threshold is not None else num_transfer_tokens[:, 0]  # (B,)
            x0, transfer_index = get_transfer_index(
                full_logits, temperature, remasking, global_mask_index, x, quota0, threshold
            )
        else:
            x0, transfer_index = get_transfer_index_dynamic(
                full_logits, temperature, remasking, global_mask_index, x, None, factor
            )
        ...
        for i in range(1, steps_per_block):
            ...
            if (x[:, s:e] == mask_id).sum() == 0:
                break
            logits_blk = _maybe_shift_logits(
                model(
                    x[:, s:e],
