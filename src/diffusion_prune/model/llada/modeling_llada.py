...
class LayerNormBase(nn.Module):
    ...
    @classmethod
    def build(cls, config: ModelConfig, size: int | None = None, **kwargs) -> LayerNormBase:
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(
        self, q: torch.Tensor, k: torch.Tensor, block_end_index: torch.Tensor | None = None
    ) -> tuple[torch.Tensor, torch.Tensor]:
...
class LLaDABlock(nn.Module):
    ...
    def set_activation_checkpointing(self, strategy: ActivationCheckpointingStrategy | None):
...
class LLaDABlockGroup(nn.ModuleList):
    def __init__(
        self, config: ModelConfig, layer_offset: int, modules: Iterable[nn.Module] | None = None
    ):
...
def create_model_config_from_pretrained_config(config: LLaDAConfig):
    ...
    Utility function
    """
    # Create a default ModelConfig to get default values for missing fields
    default_config = ModelConfig()

    kwargs = {}
    for field in fields(ModelConfig):
        # Use getattr with default from ModelConfig if attribute doesn't exist
        kwargs[field.name] = getattr(config, field.name, getattr(default_config, field.name))
...
class LLaDAModelLM(PreTrainedModel):
    ...
    _tied_weights_keys = []  # Transformers 5.0 compatibility
    all_tied_weights_keys = []

    def __init__(
        self, config: LLaDAConfig, model: LLaDAModel | None = None, init_params: bool = False
    ):
    ...
    def forward(
        ...
        | None = None,  # This is a hack mitigation of an issue in transformers `4.39.x`
        ...
        if labels is not None:
            ...
            warnings.warn(
                "Note that for LLaDA, you cannot calculate the loss here.",
                UserWarning,
                stacklevel=2,
            )
        if not return_dict:
    ...
    def prepare_inputs_for_generation(
        self, input_ids: torch.LongTensor, past_key_values: list[tuple] | None = None, **kwargs
    ):
        if past_key_values:
    ...
    def tie_weights(self, missing_keys=None, recompute_mapping=True):
        ...
        Transformers 5.0 changed the signature to include missing_keys and recompute_mapping.

        Args:
            missing_keys (list, optional): List of missing keys in the state dict.
            recompute_mapping (bool, optional): Whether to recompute the tied weights mapping.
        """
        # In LLaDA, when weight_tying is True, ff_out is not created at all.
        # The forward pass directly uses wte.weight via F.linear.
        # So there's nothing to tie here - the weights are implicitly shared.
        pass
