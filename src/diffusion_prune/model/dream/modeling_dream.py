...
class MaskedLMOutputWithPastKeyValues(MaskedLMOutput):
    def __init__(
        self, past_key_values: Optional[list[tuple[torch.Tensor, torch.Tensor]]] = None, **kwargs
    ):
...
class DreamRotaryEmbedding(nn.Module):
    @staticmethod
    def compute_default_rope_parameters(config=None, device=None, **kwargs):
        """Compute inverse frequencies for standard RoPE (no scaling)."""
        base = config.rope_theta if config is not None else kwargs.get("base", 10000)
        ...
        return inv_freq, 1.0  # attention_factor = 1.0 for default RoPE
    ...
    def __init__(
        ...
        self.config = config
        # In transformers 5.0, "default" was removed from ROPE_INIT_FUNCTIONS
...
def apply_rotary_pos_emb(
    ...
    if block_end_index is None:
        ...
        q_embed = (
            q * cos[:, :, block_end_index.item() - query_len : block_end_index.item(), :]
        ) + (
            rotate_half(q)
            * sin[:, :, block_end_index.item() - query_len : block_end_index.item(), :]
        )
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, -1, self.num_key_value_heads, self.head_dim).transpose(
            1, 2
        )
        value_states = value_states.view(
        ...
        if dual_cache:
            query_states, key_states = apply_rotary_pos_emb(
                query_states, key_states, cos, sin, block_end_index=replace_indices.max() + 1
            )
        else:
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin)
...
class DreamPreTrainedModel(PreTrainedModel):
    ...
    # Prevent from_pretrained from overwriting computed inv_freq buffers with
    # stale/zero values from the checkpoint (transformers 5.0 loads non-persistent
    # buffers if they exist in the state dict).
...
class DreamModel(DreamPreTrainedModel):
    _tied_weights_keys = None  # tie_word_embeddings=False; new transformers expects dict not list
